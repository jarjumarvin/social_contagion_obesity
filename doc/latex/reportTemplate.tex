...
TODO: describe graph generation and parameters, export function and obesity function
...
In the \textit{interact} function, if the agent is currently obese, they will recover (\textit{next\_state} becomes \textit{False}) with probability \(\mu \cdot \gamma\). 
Otherwise, if the agent is not currently obese, their initial probability to become obese is \(p = \mu \cdot \alpha\). 
For each obese friend, \(p\) increases by \(\mu \cdot \frac{\beta}{k}\). Finally, they become obese (\textit{next\_state} becomes \textit{True}) with probability \(p\). \\

Since the rates \(\gamma\) and \(\alpha\) found in the paper describing the modeling of infectious diseases\cite{infectiousDiseaseModeling} are global rates, we multiply them by some factor that is normally distributed around 1 with a (somewhat arbitrary) variance of 0.25. 
This allows us to factor in some sort of personal tendency while still somewhat preserving the global rates.
